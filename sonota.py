...
def tab3_main(tab3):
    ...
    #文字を表示する。
    param_name = tk.Label(tab3, text="Tlanslater",font=('',18),bg = bg_col)
    param_name.place(x=10, y=10)
    
    # テキストボックス
    text_box = tk.Text(tab3, width = 65, bg="#e0ffff", fg="#2f4f4f", insertbackground="#2f4f4f",
                relief="ridge", bd=5,height=10)
    
    text_box.place(x= 10, y= 40)

    ok_button = tk.Button(tab3, text="OK", width = 7, relief="solid", bd=1)
    ...
    #文字を表示する。
    param_name = tk.Label(tab3, text="出力",font=('',11), bg =  bg_col)
    param_name.place(x=10, y=218)
    textBox = Text(tab3, width=67,height=10)
